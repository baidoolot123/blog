...
class RegistrationForm(forms.ModelForm):
    email = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Введите email',}), label = '')
...
class PublishForm(forms.ModelForm):
    topic = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Введите тему',}), label = '')
    text = forms.CharField(widget=forms.TextInput(attrs={'placeholder': 'Введите публикацию',}), label = '')
    class Meta: 
        model = User 
        fields = ('email',)